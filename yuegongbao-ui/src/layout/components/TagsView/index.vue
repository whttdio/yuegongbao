...
.tags-view-container {
  ...
  border-bottom: 1px solid var(--tags-item-border);
  display: flex;
  align-items: center;
  overflow: hidden;

  $btn-width: 32px;
  $btn-color: var(--content-text-muted);
  $btn-hover-bg: var(--tags-item-hover);
  $btn-hover-color: var(--content-text);
  $btn-disabled-color: var(--panel-border-strong);
  $divider: 1px solid var(--tags-item-border);
  ...
  .tags-view-wrapper {
    ...
    .tags-view-item {
      ...
      &:first-of-type { margin-left: var(--space-1, 8px); }
      &:last-of-type  { margin-right: var(--space-2, 16px); }
    }
  }
  ...
  &:not(.tags-view-container--chrome) .tags-view-wrapper .tags-view-item.active {
    ...
    box-shadow: 0 2px 8px rgba(54, 113, 233, 0.24);
  ...
  .contextmenu {
    ...
    font-weight: 400;
    color: var(--tags-item-text);
    box-shadow: var(--gov-shadow, 0 4px 16px rgba(29, 33, 41, 0.08));
