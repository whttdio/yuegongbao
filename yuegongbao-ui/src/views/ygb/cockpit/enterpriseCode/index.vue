  ...
  <div class="app-container ygb-page">
    <section class="gov-page-header ygb-page__header">
      <div>
        ...
        <h1 class="ygb-page__title">红黄绿码企业分类</h1>
        <p class="ygb-page__desc">基于企业信用评分的红黄绿码字段展示企业风险分类。</p>
      </div>
    </section>
    ...
    <el-card class="search-card ygb-search-card" shadow="never">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
        ...
        <el-form-item label="码色" prop="colorCode">
          <el-select v-model="queryParams.colorCode" placeholder="请选择码色" clearable style="width: 140px">
    ...
    <el-card class="table-card ygb-table-card" shadow="never">
      <template #header>
        <div class="ygb-card-head ygb-card-head--between">
          <div>
            <div class="ygb-card-head__title">红黄绿码企业分类台账</div>
            <div class="ygb-card-head__desc">按企业信用评分结果归集红黄绿码分类</div>
          </div>
      ...
      <el-table v-loading="loading" :data="list">
        ...
        <el-table-column label="信用分" align="center" prop="totalScore" width="110" />
        <el-table-column label="信用等级" align="center" prop="creditLevel" width="100" />
        <el-table-column label="码色" align="center" prop="colorCode" width="100">
          <template #default="scope">
            <dict-tag :options="colorOptions" :value="scope.row.colorCode" />
          </template>
        </el-table-column>
    ...
    <page-detail-dialog v-model="detailOpen" title="红黄绿码企业详情" width="760px">
      <template v-if="detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="信用分">{{ detail.totalScore ?? '-' }}</el-descriptions-item>
          <el-descriptions-item label="信用等级">{{ detail.creditLevel || '-' }}</el-descriptions-item>
          <el-descriptions-item label="码色">{{ detail.colorCode || '-' }}</el-descriptions-item>
...
<script setup name="YgbCockpitEnterpriseCode">
...
async function getList() {
  loading.value = true
  try {
    const res = await listCreditScore(normalizedQuery())
...
async function getSummary() {
  try {
    const res = await getCreditScoreSummary(normalizedQuery())
    const data = res.data || res || {}
    const riskCount = Number(data.redCount || 0) + Number(data.yellowCount || 0) + Number(data.dCount || 0)
    summaryCards.value = [
      { key: 'totalCount', label: '参评企业', value: valueOrDefault(data.totalCount, 0), unit: '家', note: '当前筛选范围', cardClass: '' },
      { key: 'riskCount', label: '风险企业', value: riskCount, unit: '家', note: '红黄码或 D 级', cardClass: 'danger' },
      { key: 'highGradeCount', label: '优秀企业', value: valueOrDefault(data.highGradeCount, 0), unit: '家', note: 'A/B 级', cardClass: 'success' },
      { key: 'averageScore', label: '平均信用分', value: valueOrDefault(data.averageScore, 0), unit: '分', note: '平台均值', cardClass: '' }
    ]
  } catch (e) {
    console.error(e)
  }
...
function handleExport() {
  proxy.download('ygb/credit/score/export', normalizedQuery(), `cockpit_enterprise_code_${Date.now()}.xlsx`)
}
...
function normalizedQuery() {
  return {
    ...queryParams.value,
    statMonth: queryParams.value.statMonth || currentMonthText()
  }
}
