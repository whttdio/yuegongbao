  ...
  <div class="app-container ygb-page">
    <section class="gov-page-header ygb-page__header">
      <div>
        ...
        <h1 class="ygb-page__title">阳光劳务屏管理</h1>
        <p class="ygb-page__desc">管理阳光劳务屏设备台账，监控设备运行状态与展示内容。</p>
      </div>
    </section>
    ...
    <el-card class="table-card ygb-table-card" shadow="never">
      <template #header>
        <div class="ygb-card-head ygb-card-head--between">
          <div>
            <div class="ygb-card-head__title">阳光劳务屏管理台账</div>
            <div class="ygb-card-head__desc">维护阳光劳务屏设备、安装点位、在线状态和公开展示情况。</div>
          </div>
      ...
      <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="安装位置" align="center" prop="installLocation" min-width="160" />
    ...
    <page-detail-dialog v-model="detailOpen" title="阳光劳务屏管理详情" width="720px">
      <template v-if="detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="安装位置" :span="1">{{ detail.installLocation || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="授权状态" :span="2">{{ detail.authStatus || '-' }}</el-descriptions-item>
        </el-descriptions>
      </template>
    </page-detail-dialog>
  </div>
...
<script setup name="YgbDeviceSunlightScreen">
...
async function getSummary() {
  try {
    ...
    summaryCards.value = [
      ...
      { key: 'onlineCount', label: '在线', value: onlineCount, unit: '台', note: '正常运行', cardClass: 'success' },
...
function handleExport() {
  proxy.download('ygb/device/export', { ...queryParams.value }, `device_sunlight_screen_${Date.now()}.xlsx`)
}
