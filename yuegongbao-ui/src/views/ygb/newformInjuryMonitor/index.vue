...
const config = {
  ...
  tip: '聚焦平台维度的职业伤害参保异常核查，联动人员底数、参保状态和预警处置。',
  focusTitle: '监测焦点',
  focusDescription: '这个页面更偏监测视角，核心不是看平台规模，而是识别哪些平台的职业伤害保障结构失衡。',
  focusBullets: [
    { label: '已参保看覆盖', desc: '职业伤害已参保人数决定当前平台保障覆盖是否稳定。' },
  ...
  drilldown: {
    ...
    query: row => ({
      ...
      warningStatus: row.warningCount > 0 ? '1' : undefined,
      injuryInsuranceStatus: row.injuryUninsuredCount > 0 ? '0' : undefined
    })
  },
  columns: [
  ...
  summaryCards: summary => ([
    ...
    { key: 'injuryInsuredCount', topline: '稳定保障', label: '职业伤害已参保', value: summary.injuryInsuredCount ?? 0, unit: '人', cardClass: 'ygb-summary-card--success', note: '用于判断平台职业伤害保障覆盖是否稳定。' },
