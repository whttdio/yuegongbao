...
public class WorkerMessageServiceImpl implements WorkerMessageService
{
    ...
    public Map<String, Object> getLegalArticleDetail(String articleKey)
    {
        ...
        return detailWithParagraphs("法律知识文章", "法律知识",
            Arrays.asList("当前文章内容暂未同步，请稍后刷新后重试。",
                "如需立即处理问题，可先进入法律咨询或帮助中心查看对应说明。"));
    }
    ...
    {
        List<Map<String, Object>> rows = new ArrayList<>();
        rows.add(faq("faq-1", "工资被拖欠怎么办", "工资争议",
            Arrays.asList("先确认工资所属月份、应发金额和实际到账金额，并保留工资条、考勤、聊天记录等证据。",
                "如企业长期未处理，可通过法律咨询或投诉举报渠道继续提交材料并跟进处理。")));
        rows.add(faq("faq-2", "社保断缴后如何补救", "社保争议",
            Arrays.asList("先核对断缴月份、参保单位和险种，确认是否存在人员变更或申报遗漏。",
                "准备劳动合同、工资记录和社保截图后，可联系企业经办人或通过法律咨询渠道留痕处理。")));
        rows.add(faq("faq-3", "劳动合同丢失怎么处理", "劳动合同",
            Arrays.asList("可先查看电子合同、合同摘要或向企业申请调取留档副本。",
                "如合同内容存在争议，建议同步保存工资、考勤和岗位记录，必要时走法律咨询流程。")));
    ...
    public Map<String, Object> getLegalFaqDetail(String faqKey)
    {
        ...
        {
            case "faq-1" -> faq("faq-1", "工资被拖欠怎么办", "工资争议",
                Arrays.asList("先确认工资所属月份、应发金额和实际到账金额，并保留工资条、考勤、聊天记录等证据。",
                    "如企业长期未处理，可通过法律咨询或投诉举报渠道继续提交材料并跟进处理。"));
            case "faq-2" -> faq("faq-2", "社保断缴后如何补救", "社保争议",
                Arrays.asList("先核对断缴月份、参保单位和险种，确认是否存在人员变更或申报遗漏。",
                    "准备劳动合同、工资记录和社保截图后，可联系企业经办人或通过法律咨询渠道留痕处理。"));
            case "faq-3" -> faq("faq-3", "劳动合同丢失怎么处理", "劳动合同",
                Arrays.asList("可先查看电子合同、合同摘要或向企业申请调取留档副本。",
                    "如合同内容存在争议，建议同步保存工资、考勤和岗位记录，必要时走法律咨询流程。"));
            default -> null;
        };
        if (faq == null)
        {
            throw new ServiceException("未找到对应常见问题");
        }
        return faq;
    }
    ...
    private Map<String, Object> articleDetail(YgbPortalContent item)
    {
        ...
        return detailWithParagraphs(firstNonBlank(item.getTitle(), "-"),
            firstNonBlank(jsonString(extra, "typeLabel"), item.getCategoryCode(), "法律知识"), extractParagraphs(item));
    }
