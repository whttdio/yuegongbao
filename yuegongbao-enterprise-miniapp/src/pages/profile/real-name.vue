...
import { getWorkerRealnameDetail, submitWorkerRealname, uploadWorkerImage } from '../../api/enterprise-service'
...
const previewIdCardFrontUrl = computed(() => resolvePreviewImageUrl(form.idCardFrontUrl))
const previewIdCardBackUrl = computed(() => resolvePreviewImageUrl(form.idCardBackUrl))
const previewSelfieUrl = computed(() => resolvePreviewImageUrl(form.selfieUrl))

function resolvePreviewImageUrl(url) {
  const value = String(url || '')
  return value.includes('cdn.ygb.local') || value.includes('https://stub/') ? '' : value
}
