...
const snapshotPattern = /验收快照|联调快照|接口联调快照|推送本地快照/
  ...
  for (const sourceRoot of sourceRoots) {
    ...
    for (const relativeFile of walkSourceFiles(absoluteRoot)) {
      ...
      if (directStoragePattern.test(content) || baseKeyConstantPattern.test(content)) {
        failures.push(`登录态环境隔离残留：${projectFile} 仍直接读写或定义全局登录态 key`)
      }
...
function main() {
  ...
  for (const relativeFile of pageFiles) {
    ...
    if (!snapshotPattern.test(pageContent)) {
  ...
  if (pageFiles.length !== declaredRoutes.length) {
    warnings.push(`页面文件数 ${pageFiles.length} 与 pages.json 路由数 ${declaredRoutes.length} 不一致，请复核是否存在多余或缺失页面`)
  }
  ...
  console.log(`- 快照标记检查: ${pageFiles.length - failures.filter((item) => item.includes('缺少验收或联调快照标记')).length}/${pageFiles.length}`)
